...
exports.create = function(source, target, callback, noIgnore) {
  ...
  var ignoreArray;
  if (pkg && pkg.lego && pkg.lego.ignore) {
    ignoreArray = pkg.lego.ignore;
  } else {
    ignoreArray = [];
  }
  var ignoreFiles = ['.gitignore', '.legoignore'];
...
Packer.prototype.addIgnoreFiles = function () {
  ...
  // if there's a .legoignore, then we do *not* want to
  // read the .gitignore.
  if (-1 !== entries.indexOf(".legoignore")) {
